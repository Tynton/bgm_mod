...
PixelShader = 
{
	...
	[[
		float3 ApplyMonsoon( float3 vColor, float3 vPos, inout float3 vNormal, float3 vMudNormal, float4 vFoWColor, float TerrainDiffuseAlpha, float3 vMonsoonDiffuse )
		{
			...
			float vHeavy = saturate( saturate( vNoise - ( 1.0f - vIsMonsoon ) ) * 1.5f );
			float vLight = saturate( ( saturate( vNoise + 0.5f ) - ( 1.0f - vIsMonsoon ) ) * 0.5f );
			...
			vNormal = lerp( vNormal, vMudNormal, saturate( vMonsoonStrength * 0.5f ) );
			return vColor;
		}
		...
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			float3 vNormalMap = normalize( tex2D( HeightNormal, Input.uv2 ).rbg - 0.5f );
			...
			const float fTestThreshold = 0.3f;
